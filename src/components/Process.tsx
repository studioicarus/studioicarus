...
export default function Process() {
  return (
    <section id="process" className="bg-paper-light py-16 sm:py-20 lg:pb-24 lg:pt-[88px]">
      <Container>
        <Reveal>
          <SectionHeading title={<>How <BrandName /> Works</>} subtitle="A simple process. Focused results." />
        </Reveal>
          ...
          {steps.map(({ n, title, text, icon: Icon }, i) => (
            <li key={n} className={i > 0 ? 'lg:border-l lg:border-ink/20 lg:pl-10' : 'lg:pr-10'}>
              <Reveal delay={i * 90}>
                ...
                <h3 className="mt-5 text-[19px] font-semibold text-ink lg:mt-7 lg:text-[28px]">{title}</h3>
