...
export default function Process() {
  return (
    <section id="process" className="bg-white py-16 sm:py-20 lg:pb-24 lg:pt-[88px]">
      <Container>
        <Reveal>
        ...
        <ol className="mt-10 grid gap-10 sm:grid-cols-2 lg:mt-16 lg:grid-cols-4 lg:gap-0">
          {steps.map(({ n, title, text, icon: Icon }, i) => (
            <li key={n} className={i > 0 ? 'lg:border-l lg:border-slate-200/80 lg:pl-10' : 'lg:pr-10'}>
              <Reveal delay={i * 90}>
                <span className="grid h-12 w-12 place-items-center rounded-full bg-brand text-white shadow-glow lg:h-[66px] lg:w-[66px]">
                  <Icon strokeWidth={2} className="h-5 w-5 lg:h-7 lg:w-7" aria-hidden />
                </span>
                <p className="mt-5 text-[20px] font-semibold text-brand lg:mt-7 lg:text-[30px]">{n}</p>
