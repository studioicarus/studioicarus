...
export default function About() {
  return (
    <section id="about" className="bg-white py-16 sm:py-20 lg:pb-24 lg:pt-[88px]">
      <Container>
        <Reveal>
        ...
        <ul className="mt-10 grid gap-10 sm:grid-cols-2 lg:mt-16 lg:grid-cols-4 lg:gap-0">
          {aboutItems.map(({ n, title, text, icon: Icon }, i) => (
            <li key={n} className={i > 0 ? 'lg:border-l lg:border-slate-200/80 lg:pl-10' : 'lg:pr-10'}>
              <Reveal delay={i * 90}>
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-brand-50 text-brand shadow-[0_0_0_6px_rgba(240,245,255,0.8)] lg:h-[80px] lg:w-[80px] lg:rounded-[22px]">
                  <Icon strokeWidth={1.7} className="h-6 w-6 lg:h-8 lg:w-8" aria-hidden />
                </span>
                <p className="mt-5 text-[15px] font-semibold tracking-wide text-brand lg:mt-7 lg:text-[26px]">{n}</p>
