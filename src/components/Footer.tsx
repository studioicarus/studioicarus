...
export default function Footer() {
  return (
    <footer className="border-t border-black/10 bg-paper-deep text-on-deep">
      <Container inset className="grid gap-10 pb-12 pt-14 sm:grid-cols-2 lg:grid-cols-[1.55fr_1.25fr_1fr_1fr] lg:gap-8 lg:pb-16 lg:pt-20">
        <div>
          <Logo size="lg" />
          <p className="mt-6 text-[14px] text-on-deep lg:mt-8 lg:text-[18px]">Technology • Creativity • Innovation</p>
        </div>

        {footerColumns.map((col) => (
          <nav key={col.title} aria-label={col.title}>
            <h3 className="text-[15px] font-semibold text-on-deep lg:text-[20px]">{col.title}</h3>
        ...
        <div>
          <h3 className="text-[15px] font-semibold text-on-deep lg:text-[20px]">Connect</h3>
            ...
            {socials.map(({ label, href, icon: Icon }) => (
              <li key={label}>
                <a
                  ...
                  aria-label={label}
                  className="grid h-10 w-10 place-items-center rounded-full text-on-deep transition hover:bg-white/10 lg:h-11 lg:w-11"
                >
